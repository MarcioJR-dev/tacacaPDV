...
const formatarItem = (item) => {
    ...
    const valorUnit = formatarValor(item.preco);
    const total = formatarValor(item.preco * item.pedido_produto.quantidade);
    return `${item.pedido_produto.quantidade} ${nome} ${total}`;
};
